# Figure 3 — VIEM $\ell_c$ convergence on non-sphere production shapes

This document records the exact computation conditions behind
`fig3_lc_convergence_nonsphere.{png,pdf}` in `dda_results/paper/figures/`.
...
[`fig4_description.md` §3.5](fig4_description.md)) — the convergence-sweep
orientation $\beta=0$ is **not** in that grid. To make TMM available
to fig 3, the gap is filled by a dedicated EBCM run at $(a_{\rm eq}=0.1,
...
- **One figure file**: `fig3_lc_convergence_nonsphere.{png,pdf}`.
...
discussion in [`fig3_description.md §6 / §9`](fig3_description.md). The Au
panels in fig 3 are therefore re-run with **block-Krylov subspace expansion**
...
omitted from fig 3 simply to make the dual TMM ⟷ Richardson overlay
...
0.2\,\mu m$, propagating to fig 4's `oblate × |S_bk|` scatter panels.
...
- `dda_results/paper/figures/fig3_lc_convergence_nonsphere.{png,pdf}`
...
$$
The slope-$-2/3$ guide on each fig 3 panel is this asymptotic prediction.
...
| Series | $\|r\|_F/\|b\|_F\big|_{\rm fine}$ | $\|r\|_F/\|b\|_F\big|_{\rm 2nd}$ | Pass $10^{-3}$? | fig 3 action |
| --- | ---: | ---: | :---: | --- |
...
[`fig5_description.md`](fig5_description.md) for sphere RHS scaling;
...
the clean asymptotic regime. None of the n15 / n20 panels in fig 3
...
slope guide in the gre $\times$ n15 panel of fig 3. The
...
- [`fig1_description.md`](fig1_description.md) — sphere convergence study;
  fig 3 is the non-sphere extension.
- [`fig4_description.md`](fig4_description.md) — production-sweep
  correctness against TMM / MSTM exact references; fig 3 fills the
  discretization-rate axis that fig 4 cannot expose at a single
  $\ell_c$ choice.
