...
- **Single observable** (`C_ext`) = **1 figure file**:
  `fig3_scatter_C_ext_rve0.1.{png,pdf}`.
...
- Reduced from the earlier 4-observable layout (`C_ext`, `|S_fw^θ|`,
  ...
  for `Re/Im S_fw^θ` is already covered by fig 1 (sphere) and fig 2
  (non-sphere), and Galerkin theory implies the same h² / h convergence
  rate for any bounded linear functional.  The remaining role of fig 3
...
- $\beta$ markers: 3 distinct shapes for $\cos\beta \in \{+0.8, +0.4, 0\}$.
  The $\cos\beta < 0$ hemisphere is omitted in fig 3 because the
