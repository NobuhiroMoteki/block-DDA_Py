# Figure 4 — VIEM $\ell_c$ convergence on non-sphere production shapes

This document records the exact computation conditions behind
`fig4_lc_convergence_nonsphere.{png,pdf}` in `dda_results/paper/figures/`.
...
to fig 4, the gap is filled by a dedicated EBCM run at $(a_{\rm eq}=0.1,
...
- **One figure file**: `fig4_lc_convergence_nonsphere.{png,pdf}`.
...
omitted from fig 4 simply to make the dual TMM ⟷ Richardson overlay
...
- `dda_results/paper/figures/fig4_lc_convergence_nonsphere.{png,pdf}`
...
$$
The slope-$-2/3$ guide on each fig 4 panel is this asymptotic prediction.
...
2. **Shape-universality** of the same $h^2$ rate on the headline
   observable $|S_{\rm fw}^{\theta}|$ (fig 4, oblate + GRE).
...
[`fig2_description.md §6`](fig2_description.md) — but only a small
...
| Series | $\|r\|_F/\|b\|_F\big|_{\rm fine}$ | $\|r\|_F/\|b\|_F\big|_{\rm 2nd}$ | Pass $10^{-3}$? | fig 4 action |
| --- | ---: | ---: | :---: | --- |
...
The GRE × Au panel of fig 4 is the only one in the figure with no
...
[`fig2_description.md §6`](fig2_description.md) for sphere RHS scaling;
...
the clean asymptotic regime. None of the n15 / n20 panels in fig 4
...
slope guide in the gre $\times$ n15 panel of fig 4. The
...
- [`fig1_description.md`](fig1_description.md) — sphere convergence study;
  fig 4 is the non-sphere extension.
- [`fig3_description.md`](fig3_description.md) — production-sweep
  correctness against TMM / MSTM exact references; fig 4 fills the
