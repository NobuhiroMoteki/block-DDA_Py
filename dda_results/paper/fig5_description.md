# Figure 5 — Block-Krylov RHS scaling: residual traces and per-RHS speedup

This document records the exact computation conditions behind
`fig5_rhs_scaling.{png,pdf}` and the physical / numerical
...
- **Row 2 (bottom): per-RHS speedup** $t(L=1)/t(L)$ where $t(L)$ is the
  ...
  gate also used by fig 3 / fig 4). The ideal amortisation line
...
**Single figure file**: `fig5_rhs_scaling.{png,pdf}`, **2 × 3 panels**
...
fig 3 (VIEM $\ell_c$ convergence) and fig 4 (production scatter), so
...
This is **different from fig 4's spheroid-mode grid**
...
- `dda_results/paper/figures/fig5_rhs_scaling.{png,pdf}`
