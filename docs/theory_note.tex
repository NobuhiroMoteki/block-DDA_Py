...
\author{N.\ Moteki}
\date{Last updated: 2026-03-28 (corresponds to block-DDA\_Py v0.7.1)}
...
The vacuum wavelength $\lambda_0$ and medium refractive index $m_m$
are fixed parameters (stored as HDF5 root attributes, not swept).
The imaginary refractive index $\operatorname{Im}(m_p)$ is also fixed
(default~0).  The total number of DDA conditions is
$N_{D_{ve}} \times N_{\mathrm{RI}} \times N_{\mathrm{AR}}$.
...
All datasets reside in the HDF5 root group (no subgroups).

\begin{itemize}
  \item \textbf{Grid axis arrays} (1D, equidistant):
  ...
  \item \textbf{Convergence flag}
        (shape $(N_{D_{ve}}, N_{\mathrm{RI}}, N_{\mathrm{AR}})$,
        dtype bool):
    \texttt{converged}.  \texttt{True} where the DDA solver converged.

  \item \textbf{Root attributes} (provenance):
    \texttt{wl\_0}, \texttt{m\_m}, \texttt{m\_imag},
