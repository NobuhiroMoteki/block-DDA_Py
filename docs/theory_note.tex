...
\author{N.\ Moteki}
\date{Last updated: 2026-03-26 (corresponds to block-DDA\_Py v0.5.0)}
...
circular.
This setting is hard-coded in the current version (v0.5.0).
...
In the current version (v0.5.0), $\ell_c = 0.3\,c$ is adopted as an
empirical choice.
...
\begin{itemize}
  \item Only $\beta$ (polar angle) is sampled, with
        $\cos\beta \sim \mathrm{Uniform}(-1,1)$ for uniform coverage
        on the sphere; $\alpha = 0$ and $\gamma = 0$ are fixed.
  \item The phi-averaged $S_s = S_p$ is computed analytically via
        \eqref{eq:S-phi-avg} using
        \texttt{compute\_phi\_averaged\_PCAS\_S\_fw()} in
        \texttt{bl\_dda/scatterer.py}.
  \item $C_{\mathrm{ext}}$ and $C_{\mathrm{abs}}$ are used directly
        from the $\alpha = 0$ solution.
\end{itemize}
This reduces the number of independent DDA solves from
$N_\theta \times N_\phi$ to $N_\theta$, while providing
\emph{exact} (not approximate) $\alpha$-averaged observables.
